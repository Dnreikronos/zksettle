...
Current mitigation in `use-prove-flow.ts` finalize batch: `setComputeUnitPrice({ microLamports: 50_000 })` (×default 200K CU ≈ 10,000 lamports ≈ 0.00001 SOL) + `sendSigned(..., { maxRetries: 10 })`. The `confirmTx` fallback (`use-prove-flow.ts:269-302`) additionally polls `getSignatureStatus` for 30s after the blockhash strategy returns `BlockheightExceeded`, catching late inclusions. Together these reduce false-negative expiry errs to near-zero under normal devnet load. **Do not** drop either without re-benchmarking against a busy devnet slot; the failure mode is silent (sig never lands) and very expensive to debug because both Alchemy logs and Solscan show no record of the tx.

## Docker build context ignores `.gitignore` — always need a `.dockerignore`

`.gitignore` does NOT influence the docker build context. With `context: .` in `docker-compose.yml` and no `.dockerignore`, docker tarballs the entire repo — including `backend/target/` (~56 GB cargo cache), `node_modules`, `frontend/.next`, etc. Symptom: `docker compose build` runs at 99% CPU for 45+ minutes with no visible progress (the `Sending build context to Docker daemon` line keeps growing past 75 MB and the daemon eventually OOMs or fills `/var/lib/docker`).

Fix: keep `.dockerignore` at the repo root in sync with what each Dockerfile actually needs. For the backend image we need `backend/{Cargo.toml,Cargo.lock,rust-toolchain.toml,programs/,crates/}` plus `circuits/target/zksettle_slice.{json,ccs,pk}` — exclude everything else, especially the four heavy dirs above. If a future service needs additional context, *add* a whitelist entry; do not delete the broad excludes. Re-included paths (whitelists for the circuit artefacts) must come **after** the broad excludes — order matters in dockerignore. Reproduce the slowdown trivially by removing the file and running `docker compose build issuer-service` — context size will start climbing through GBs immediately.
